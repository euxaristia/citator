...
Deno.test("DailyVerseScheduler - constructor", () => {
  ...
  const sendToChannel = async (_channelId: string, _embed: EmbedBuilder) => {};
...
Deno.test("DailyVerseScheduler - triggerNow", async () => {
  ...
  const sendToChannel = async (id: string, embed: EmbedBuilder) => {
    called = true;
    channelId = id;
    embedTitle = embed.toJSON().title || "";
  };
  ...
  assertEquals(embedTitle, "Daily Verse");
});
